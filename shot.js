...
(async () => {
  ...
  await shot('6helped');  await pick('opts6','確定申告のことを教えてもらえた'); await next();
  await shot('7income');  await pick('opts7','おおよその幅なら伝えてよい'); await next();
  await shot('8loan');    await pick('opts8','減ったか変わらないか、だけなら伝えてよい'); await next();
