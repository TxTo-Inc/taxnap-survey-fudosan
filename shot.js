...
(async () => {
  ...
  await pick('opts4','本当に利益が出ているのか分からない');
  await pick('opts4','その他');
  await shot('4worry');
  await p.fill('#q4Other','管理組合の修繕計画'); await next();
  await shot('5freq');    await pick('opts5','数ヶ月に1回程度'); await next();
  await shot('6welcome'); await pick('opts6','収支や税金で確認したほうがいい点の知らせ'); await next();
  await shot('7concern'); await pick('opts7','電話で来るのが困る'); await next();
  await shot('8share');   await pick('opts8','確定申告が終わったかどうか'); await next();
  await shot('9interview');
  await pick('opts9','協力できる');
  await p.fill('#email','test@example.com');
  await p.click('#next9'); await p.waitForTimeout(800);
  await shot('10done');
